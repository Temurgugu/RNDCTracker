...
InterestInfluenceMatrix <- stakeholder %>%
  ...
  scale_size_continuous(breaks=seq(0, 10, 2), limits = c(0,10), name = "Influence Rate", range = c(-5,10))+
  labs(title="Interest–influence matrix for NDC Stakeholders",
       subtitle ="")
  
  
InterestInfluenceMatrix

#Save the ggplot
ggsave("visvalisation/InterestInfluenceMatrix.png", 
       plot = InterestInfluenceMatrix,
       width = 24, 
       height = 20, 
       units = "cm")  
